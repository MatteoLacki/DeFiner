...
MatrixPriorityQueue <- setRefClass(
    "MatrixPriorityQueue",
    ...
    methods     = list(
        ...
        initialize  = function( 
            ...
            if( !is.null( configuration ) ){
                ...
                push( configuration, logProb )
            }
            invisible()
        },

        enqueue = function( configurations ){
            apply(
                configurations, 2, 
                function( configuration ){
                    probInd     <- length(configuration)
                    push(
                        item    = configuration[-probInd ],
                        priority= configuration[ probInd ]
                    )
                    invisible()
                }
            )
